...
> Note: Since the device model is shared among all the devices, the ID should be unique. For convinience, the ID is generated by the application. By using the API, the ID is not required, but if provided, it will be used as the ID of the device model.
