...
namespace IoTHub.Portal.Infrastructure.AWS.Services
{
    ...
    public class ConfigService : IConfigService
    {
        ...
        public async Task<string> RollOutEdgeModelConfiguration(IoTEdgeModelDto edgeModel)
        {
            ...
            try
            {
                var createDeploymentResponse = await greengrass.CreateDeploymentAsync(createDeploymentRequest);
        ...
        private async Task<string> GetThingGroupArn(IoTEdgeModelDto edgeModel)
        {
            ...
            try
            {
                var existingThingGroupResponse = await iotClient.DescribeThingGroupAsync(dynamicThingGroup);
            ...
            catch (Amazon.IoT.Model.ResourceNotFoundException)
            {
                try
                {
                    var createThingGroupResponse = await iotClient.CreateDynamicThingGroupAsync(new CreateDynamicThingGroupRequest
                    {
                        ThingGroupName = edgeModel!.Name,
        ...
        private async Task CreateThingTypeIfNotExists(string thingTypeName)
        {
            ...
            try
            {
                _ = await iotClient.DescribeThingTypeAsync(existingThingType);
            }
            catch (Amazon.IoT.Model.ResourceNotFoundException)
            {
                try
                {
                    _ = await iotClient.CreateThingTypeAsync(new CreateThingTypeRequest
                    {
                        ThingTypeName = thingTypeName,
                        Tags = new List<Tag>
                    {
        ...
        private async Task<Dictionary<string, ComponentDeploymentSpecification>> CreateGreenGrassComponents(IoTEdgeModelDto edgeModel)
        {
            ...
            foreach (var component in edgeModel.EdgeModules)
            {
                try
                {
                    ...
                    _ = await greengrass.DescribeComponentAsync(new DescribeComponentRequest
                    {
                        Arn = componentArn
                    });
        ...
        public async Task DeleteConfiguration(string modelId)
        {
            ...
            {
                logger.LogError($"Failed to get model modules when deleting. Some resources might persist in AWS for model {modelId}.");
            }
            ...
            foreach (var module in modules.Where(c => string.IsNullOrEmpty(c.Id)))
            {
                try
                {
                    _ = await greengrass.DeleteComponentAsync(new DeleteComponentRequest
                    {
            ...
            try
            {
                _ = await greengrass.CancelDeploymentAsync(new CancelDeploymentRequest
                {
            ...
            try
            {
                _ = await greengrass.DeleteDeploymentAsync(new DeleteDeploymentRequest
                {
                    DeploymentId = modelId
                });
            }
        ...
        private async Task DeprecateDeploymentThingType(string modelId)
        {
            try
            {
                var deployment = await greengrass.GetDeploymentAsync(new GetDeploymentRequest
                {
                    DeploymentId = modelId
                });
                ...
                try
                {
                    _ = await iotClient.DeprecateThingTypeAsync(new DeprecateThingTypeRequest
                    {
                ...
                {
                    logger.LogWarning($"Failed to depreciate thing type {deployment.DeploymentName} since it does'nt exist.");
                }
                catch (AmazonIoTException e)
                {
        ...
        public async Task<int> GetFailedDeploymentsCount()
        {
            ...
            try
            {
                var deployments = await greengrass.ListDeploymentsAsync(new ListDeploymentsRequest
                {
        ...
        public async Task<List<IoTEdgeModuleDto>> GetConfigModuleList(string modelId)
        {
            ...
            try
            {
                var response = await greengrass.GetDeploymentAsync(getDeployement);
                ...
                foreach (var compoenent in response.Components)
                {
                    ...
                    try
                    {
                        var responseComponent = await greengrass.GetComponentAsync(new GetComponentRequest
                        {
        ...
        public async Task<IEnumerable<IoTEdgeModuleDto>> GetPublicEdgeModules()
        {
            ...
            do
            {
                try
                {
                    var response = await greengrass.ListComponentsAsync(new ListComponentsRequest
                    {
