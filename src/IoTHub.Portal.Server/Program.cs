...
namespace IoTHub.Portal.Server
{
    public static class Program
    {
        ...
        private static void ConfigureServices(IServiceCollection services)
        {
            ...
            var configuration = ConfigHandlerFactory.Create(builder.Environment, builder.Configuration);
    ...
    > Ex: ``Authorization: Bearer * ***``",
                ...
                var securityRequirements = new OpenApiSecurityRequirement
        {
            ...
            _ = services.AddQuartz(q =>
            {
                ...
                q.UsePersistentStore(opts =>
                {
                    ...
                    if (configuration.DbProvider != DbProviders.SqLite)
                        opts.UseClustering();
                    ...
                    switch (configuration.DbProvider)
                    {
                        ...
                        case DbProviders.SqLite:
                            opts.UseMicrosoftSQLite(c =>
                            {
                                c.ConnectionString = configuration.SQLiteConnectionString;
                            });
                            break;
                        default:
                            break;
            ...
            _ = services.AddQuartzHostedService(q =>
            {
        ...
        private static void Configure(WebApplication webApp, IWebHostEnvironment env)
        {
            ArgumentNullException.ThrowIfNull(env, nameof(env));
            ArgumentNullException.ThrowIfNull(webApp, nameof(webApp));

            var configuration = webApp.Services.GetService<ConfigHandler>();

            // Use problem details
            _ = webApp.UseProblemDetails();
            webApp.UseIfElse(IsApiRequest, UseApiExceptionMiddleware, UseUiExceptionMiddleware);

            if (configuration.UseSecurityHeaders)
            {
                _ = webApp.UseSecurityHeaders(opts =>
                {
...
#pragma warning restore ASP0018 // Unused route parameter
            ...
            {
                case CloudProviders.Azure:
                    ConfigureAzureAsync(webApp);
                    break;
                case CloudProviders.AWS:
                    ConfigureAwsAsync(webApp);
                    break;
                default:
                    // Throw error
                    throw new InvalidOperationException($"Cloud provider {configuration.CloudProvider} is not supported");
            }
        }

        private static void ConfigureAzureAsync(IApplicationBuilder webApp)
        {
            var deviceModelImageManager = webApp.ApplicationServices.GetService<IDeviceModelImageManager>();

            deviceModelImageManager?.InitializeDefaultImageBlob().Wait();
            deviceModelImageManager?.SyncImagesCacheControl().Wait();
        }

        private static void ConfigureAwsAsync(IApplicationBuilder webApp)
        {
            var deviceModelImageManager = webApp.ApplicationServices.GetService<IDeviceModelImageManager>();
        ...
        private static void UseApiExceptionMiddleware(IApplicationBuilder webApp)
        {
            _ = webApp.UseProblemDetails();
        }

        private static void UseUiExceptionMiddleware(IApplicationBuilder webApp)
        {
            _ = builder.Environment.IsDevelopment() ? webApp.UseDeveloperExceptionPage() : webApp.UseExceptionHandler("/Error");
        }
