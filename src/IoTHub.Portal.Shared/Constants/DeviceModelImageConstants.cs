// Copyright (c) CGI France. All rights reserved.
// Licensed under the MIT license. See LICENSE file in the project root for full license information.

namespace IoTHub.Portal.Shared.Constants
{
    public static class DeviceModelImageConstants
    {
        /// <summary>
        /// The base64 placeholder image returned by the API when no custom avatar has been set for a device model.
        /// Must match IoTHub.Portal.Domain.Options.DeviceModelImageOptions.DefaultImage.
        /// </summary>
        public const string DefaultImage =
            "data:image/png;base64,";
    }
}
